...
class Embedbase:
    ...
    async def add(
        ...
        df['hash'] = df.apply(
            lambda row: hashlib.sha256(
                # Kombinace dat a metadat seřazených podle klíčů
                f"{row['data']}{json.dumps(row['metadata'], sort_keys=True) if row['metadata'] else ''}".encode()
            ).hexdigest(),
            axis=1
        )
        ...
        self.logger.info(f"Found {len(existing_documents)} cached documents")
    ...
    async def update(
        ...
        self.logger.info(f"Found {len(existing_embeddings)} cached documents")
    ...
    async def semantic_search(
        ...
        for match in query_response:
            self.logger.info(f"app.py - query found answer in document: {match.metadata}. Score: {match.score}")
